...
class FamilyMatchGame extends StatefulWidget {
  const FamilyMatchGame({super.key});

  @override
  State<FamilyMatchGame> createState() => _FamilyMatchGameState();
}

class _FamilyMatchGameState extends State<FamilyMatchGame> {
  final r = Random();
  ...
  void _setup() {
    final pool = [...kFamilyVocab]..shuffle(r);
    final take = pool.take(10).toList();
    _pairs = take.map((v) => Pair(left: v.term, right: v.indo, explain: '“${v.term}” berarti “${v.indo}”.')).toList();

    int id = 1;
    _choices = take.expand((v) => [Choice(id: id++, text: v.indo, key: v.term)]).toList()..shuffle(r);
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          infoBadge(icon: Icons.touch_app, text: 'Pilih TERJEMAHAN (chips) di bawah, lalu ketuk KARTU kata Inggris yang cocok.'),
                ...
                return InkWell(
                  ...
                  child: Container(
                    ...
                    child: ListTile(
                      ...
                      trailing: Icon(Icons.translate, color: color),
                    ),
                  ),
                );
              },
          ...
          const SizedBox(height: 8),
          Text('Pilih terjemahan:', style: primaryTextStyle.copyWith(fontWeight: semiBold)),
          const SizedBox(height: 6),
  ...
  void _onTap(Pair p) {
    if (_selectedId == null) {
      AwesomeDialog(
        ...
        title: 'Pilih dulu',
        desc: 'Tap salah satu terjemahan (chips), lalu klik kartu kata Inggris yang ingin kamu jawab.',
        btnOkOnPress: () {},
      ).show();
      return;
    }
