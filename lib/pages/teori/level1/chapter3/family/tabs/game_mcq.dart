...
class FamilyMcqGame extends StatefulWidget {
  const FamilyMcqGame({super.key});

  @override
  State<FamilyMcqGame> createState() => _FamilyMcqGameState();
}

class _FamilyMcqGameState extends State<FamilyMcqGame> {
  final r = Random();
  ...
  void _generate() {
    ...
    _items = take.map((v) {
      final wrongs = (kFamilyVocab.where((x) => x.indo != v.indo).toList()..shuffle(r)).take(3).map((e) => e.indo).toList();
