...
class GestureMatchGame extends StatefulWidget {
  const GestureMatchGame({super.key});
  @override
...
class _GestureMatchGameState extends State<GestureMatchGame> {
  ...
  void _setup() {
    final pool = List<GestureVocab>.of(kGestureVocab)..shuffle(r);
    final take = pool.take(12).toList();
    _pairs = take.map((v) => Pair(left: '${v.emoji} ${v.term}', right: v.meaning, explain: '"${v.term}" ≈ ${v.meaning}')).toList();
    int id = 1;
    _choices = take.map((v) => Choice(id: id++, text: v.meaning, key: '${v.emoji} ${v.term}')).toList()..shuffle(r);
    _marks = {for (final p in _pairs) p.left: Mark.neutral};
    _selectedId = null; setState((){});
  }
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          infoBadge(icon: Icons.touch_app, text: 'Pilih ARTI (chips), lalu ketuk KARTU gesture yang cocok.'),
                ...
                return InkWell(
                  ...
                  child: Container(
                    ...
                    child: ListTile(
                      ...
                      trailing: Icon(Icons.gesture, color: color),
                    ),
                  ),
                );
              },
          ...
          //       ChoiceChip(
          //         label: Text(c.text, overflow: TextOverflow.ellipsis),
