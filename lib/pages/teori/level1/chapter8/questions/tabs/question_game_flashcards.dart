...
class QuestionFlashcardGame extends StatefulWidget {
  const QuestionFlashcardGame({super.key});
  @override
  State<QuestionFlashcardGame> createState() => _QuestionFlashcardGameState();
}

class _QuestionFlashcardGameState extends State<QuestionFlashcardGame> {
  late List<QWVocab> _queue;
  int _index = 0;
  int _known = 0;
  @override
  ...
  void _reset() {
    _queue = List<QWVocab>.of(kQWVocab)..shuffle();
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text('Hafal: $_known / ${kQWVocab.length}', style: primaryTextStyle),
              ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _front
              ? Text(widget.v.word, style: primaryTextStyle.copyWith(fontWeight: FontWeight.w700), textAlign: TextAlign.center, maxLines: 3, overflow: TextOverflow.ellipsis)
              : Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${widget.v.indo}${widget.v.note != null ? ' • ${widget.v.note}' : ''}', style: primaryTextStyle, textAlign: TextAlign.center, maxLines: 4, overflow: TextOverflow.ellipsis),
              if (widget.v.example != null) ...[
                const SizedBox(height: 8),
                Text('e.g. ${widget.v.example!}', style: primaryTextStyle.copyWith(fontSize: 12, color: Colors.grey[700]), textAlign: TextAlign.center, maxLines: 4, overflow: TextOverflow.ellipsis),
              ],
            ],
          ),
        ),
