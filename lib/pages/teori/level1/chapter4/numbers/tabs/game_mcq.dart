...
class NumbersMcqGame extends StatefulWidget {
  const NumbersMcqGame({super.key});

  @override
  State<NumbersMcqGame> createState() => _NumbersMcqGameState();
}

class _NumbersMcqGameState extends State<NumbersMcqGame> {
  final r = Random();
  ...
  void _generate() {
    ...
    _items = take.map((v) {
      final wrongs = (kNumberVocab.where((x) => x.word != v.word).toList()..shuffle(r))
          .take(3).map((e) => e.word).toList();
      final opts = [...wrongs, v.word]..shuffle(r);
      final correct = opts.indexOf(v.word);
      return MCQItem(
        prompt: 'Ejaan Inggris dari angka ${v.numeral} adalah…',
        options: opts,
        correct: correct,
        explain: '“${v.numeral}” → “${v.word}”. ${v.example ?? ''}',
      );
    }).toList();
    _selected.clear();
    setState(() {});
