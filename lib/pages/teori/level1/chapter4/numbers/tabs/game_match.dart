...
class NumbersMatchGame extends StatefulWidget {
  const NumbersMatchGame({super.key});

  @override
  State<NumbersMatchGame> createState() => _NumbersMatchGameState();
}

class _NumbersMatchGameState extends State<NumbersMatchGame> {
  final r = Random();
  ...
  void _setup() {
    final pool = [...kNumberVocab]..shuffle(r);
    final take = pool.take(12).toList();
    _pairs = take.map((v) => Pair(left: v.numeral, right: v.word, explain: '“${v.numeral}” dibaca “${v.word}”.')).toList();

    int id = 1;
    _choices = take.map((v) => Choice(id: id++, text: v.word, key: v.numeral)).toList()..shuffle(r);
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          infoBadge(icon: Icons.touch_app, text: 'Pilih EJAAN (chips), lalu ketuk KARTU angka yang cocok.'),
                ...
                return InkWell(
                  ...
                  child: Container(
                    ...
                    child: ListTile(
                      ...
                      trailing: Icon(Icons.numbers, color: color),
                    ),
                  ),
                );
              },
          ...
          const SizedBox(height: 8),
          Text('Pilih ejaan:', style: primaryTextStyle.copyWith(fontWeight: FontWeight.w700)),
          const SizedBox(height: 6),
          ...
          // SizedBox(
          //   // atur tinggi sesuai kebutuhan layar
          //   height: 140,
